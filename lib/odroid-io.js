...
function read(instance) {
  ...
  if (!read._samplingInterval) {
    read._samplingInterval = 1;
  }
  read._isReading = true;
  read._interval = setInterval(function() {
    // Process Digital
    processDigital.bind(instance)();
    processAnalog.bind(instance)();
  }, read._samplingInterval);
}
...
OdroidIO.prototype.setSamplingInterval = function(ms) {
  ...
  read._samplingInterval = this._samplingInterval;
  clearInterval(read._interval);
  read._isReading = false;
  read(this);
}
...
OdroidIO.prototype.i2cWrite = function(address, cmdRegOrData, inBytes) {
  var i2c = this._i2cBuses[this._addressToBus[address]] || this._i2cBuses[this._defaultI2cBus];
